...
public class SaleUpdatedEventConsumerTests
{
    private readonly ISaleRepository _saleRepository;
    private readonly SaleUpdatedEventConsumer _consumer;
    private readonly ISaleEventRepository _saleEventRepository;

    public SaleUpdatedEventConsumerTests()
    {
        var logger = new LoggerFactory().CreateLogger<SaleUpdatedEventConsumer>();

        _saleRepository = Substitute.For<ISaleRepository>();
        _saleEventRepository = Substitute.For<ISaleEventRepository>();
        _consumer = new SaleUpdatedEventConsumer(_saleRepository, _saleEventRepository, logger);
    }
    ...
    public async Task Given_ExistingSale_When_Consumed_Then_ShouldSaveInTheSaleEventRepository()
    {
        ...
        var context = Substitute.For<ConsumeContext<UpdateSaleMessage>>();
        context.Message.Returns(new UpdateSaleMessage(sale.Id, DateTime.UtcNow));
    ...
    public async Task Given_NotFoundSale_When_Consumed_Then_ShouldThrowException()
    {
        ...
        var context = Substitute.For<ConsumeContext<UpdateSaleMessage>>();
        context.Message.Returns(new UpdateSaleMessage(Guid.Empty, DateTime.UtcNow));
