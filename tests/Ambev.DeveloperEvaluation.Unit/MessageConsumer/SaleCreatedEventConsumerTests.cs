...
public class SaleCreatedEventConsumerTests
{
    private readonly ISaleRepository _saleRepository;
    private readonly SaleCreatedEventConsumer _consumer;
    private readonly ISaleEventRepository _saleEventRepository;

    public SaleCreatedEventConsumerTests()
    {
        var logger = new LoggerFactory().CreateLogger<SaleCreatedEventConsumer>();

        _saleRepository = Substitute.For<ISaleRepository>();
        _saleEventRepository = Substitute.For<ISaleEventRepository>();
        _consumer = new SaleCreatedEventConsumer(_saleRepository, _saleEventRepository, logger);
    }
    ...
    public async Task Given_ExistingSale_When_Consumed_Then_ShouldSaveInTheSaleEventRepository()
    {
        ...
        var context = Substitute.For<ConsumeContext<CreateSaleMessage>>();
        context.Message.Returns(new CreateSaleMessage(sale.Id, DateTime.UtcNow));
    ...
    public async Task Given_NotFoundSale_When_Consumed_Then_ShouldThrowException()
    {
        ...
        var context = Substitute.For<ConsumeContext<CreateSaleMessage>>();
        context.Message.Returns(new CreateSaleMessage(Guid.Empty, DateTime.UtcNow));
